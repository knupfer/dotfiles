...
(eval-after-load 'w3m
  '(progn
     (defun knu-orgify-wikipedia ()
       (save-excursion
         (while (re-search-forward " +$" nil t)
           (replace-match "")))
       (when (string-match "en\.wikipedia" w3m-current-url)
         (require 'org)
         (save-excursion
           (setq contents nil)
           (goto-char (point-min))
           ...
           (let ((begin (match-beginning 0)))
             ...
             (let ((end (point)))
               ...
               (while
                 ...
                 (add-to-list 'contents (list (/ (1+ (length (match-string-no-properties 1))) 2) (match-string-no-properties 2)) t))
               (delete-region begin end)))
           (goto-char (point-min))
           (while contents
             (let ((level (caar contents))
                   (heading (cadr (pop contents))))
...
(eval-after-load 'w3m
  '(mapc (lambda (x) (define-key w3m-mode-map (kbd (car x)) (cadr x)))
         '(("M-<left>" w3m-view-previous-page)
           ...
           ("<tab>" w3m-next-buffer)
           ("<S-iso-lefttab>" w3m-previous-buffer)
...
#+BEGIN_SRC emacs-lisp
(add-hook 'w3m-fontify-after-hook 'knu-orgify-wikipedia)
