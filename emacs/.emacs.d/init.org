...
(mapc (lambda (x) (add-to-list 'pretty-symbol-patterns x))
      '((?α knu-custom "\\\\alpha" (org-mode latex-mode))
        ...
        (?ℝ knu-custom "\\\\mathbb{R}" (org-mode latex-mode))
        ...
        (?⊥ knu-custom "\\\\perp" (org-mode latex-mode))
        (?❰ knu-custom "\\\\left(" (org-mode latex-mode))
        (?❱ knu-custom "\\\\right)" (org-mode latex-mode))
        (?【 knu-custom "\\\\left[" (org-mode latex-mode))
        (?】 knu-custom "\\\\right]" (org-mode latex-mode))
        (?⁰ knu-custom "\\^0" (org-mode latex-mode))
        (?ⁱ knu-custom "\\^1" (org-mode latex-mode))
        (?² knu-custom "\\^2" (org-mode latex-mode))
        (?³ knu-custom "\\^3" (org-mode latex-mode))
        (?⁴ knu-custom "\\^4" (org-mode latex-mode))
        (?⁵ knu-custom "\\^5" (org-mode latex-mode))
        (?⁶ knu-custom "\\^6" (org-mode latex-mode))
        (?⁷ knu-custom "\\^7" (org-mode latex-mode))
        (?⁸ knu-custom "\\^8" (org-mode latex-mode))
        (?⁹ knu-custom "\\^9" (org-mode latex-mode))
        (?⁺ knu-custom "\\^\\+" (org-mode latex-mode))
        (?⁻ knu-custom "\\^-" (org-mode latex-mode))
        (?ⁿ knu-custom "\\^n" (org-mode latex-mode))
        (?√ knu-custom "\\\\sqrt" (org-mode latex-mode))
        (?λ knu-custom "\\\\lambda" (org-mode latex-mode))
        (?∫ knu-custom "\\\\int" (org-mode latex-mode))
        (?∈ knu-custom "\\\\in" (org-mode latex-mode))
        (?Λ knu-custom "\\\\Lambda" (org-mode latex-mode))
...
;        (?Ø knu-custom "()" (haskell-mode))
;        (?Ø knu-custom "\\[\\]" (haskell-mode))
;        (?─ knu-custom "--" (haskell-mode))
;        (?∃ knu-custom "exists" (haskell-mode))
;        (?⋙ knu-custom ">>>" (haskell-mode))
;        (?⋘ knu-custom "<<<" (haskell-mode))
;        (?⪜ knu-custom ">>=" (haskell-mode))
;        (?⋇ knu-custom "<\\*>" (haskell-mode))
;        (?∑ knu-custom "sum" (haskell-mode))
;        (?∀ knu-custom "forall" (haskell-mode))
;        (?→ knu-custom "->" (haskell-mode))
;        (?← knu-custom "<-" (haskell-mode))
        (?∷ knu-custom "::" (haskell-mode))
;        (?⇛ knu-custom "==>" (haskell-mode))
...
;        (?≤ knu-custom "<=" (haskell-mode))
;        (?⇒ knu-custom "=>" (haskell-mode))
;        (?∈ knu-custom "`elem`" (haskell-mode))
;        (?∈ knu-custom "`member`" (haskell-mode))
;        (?∉ knu-custom "`notMember`" (haskell-mode))
;        (?∉ knu-custom "`notElem`" (haskell-mode))
;        (?⧺ knu-custom "++" (haskell-mode))
;        (?∘ knu-custom " \\. " (haskell-mode))
;        (?≢ knu-custom "/=" (haskell-mode))
;        (?… knu-custom "\\.\\." (haskell-mode))
;        (?∨ knu-custom "||" (haskell-mode))
;        (?∧ knu-custom "&&" (haskell-mode))
;        (?ℓ knu-custom "length" (haskell-mode))
;        (?⅏ knu-custom "String" (haskell-mode))
;        (?ℶ knu-custom "where" (haskell-mode))
        (?ℤ knu-custom "Integer" (haskell-mode))
        (?ℤ knu-custom "Int" (haskell-mode))
        (?ℚ knu-custom "Double" (haskell-mode))
        (?ℚ knu-custom "Float" (haskell-mode))
